...
import { generateRoutine as generateAIRoutine } from '@/services/routineGenerator';
...
export default function RoutineBuilder() {
  ...
  // Additional error handling to add to your generateRoutine function
...
const generateRoutine = async () => {
  ...
  try {
    ...
    toast({
      ...
      description: 'Your personalized AI routine is ready.'
    });
  } catch (error: any) {
    ...
    if (error.message?.includes('API')) {
      toast({
        ...
        description: 'Unable to connect to AI service. Using fallback method instead.',
        variant: 'destructive'
      });
      
      // Try fallback
      try {
        const fallbackRoutine = createEnhancedRoutine(answers);
  ...
  const aiGenerateRoutine = async (answers: Record<string, any>): Promise<Routine> => {
    try {
      const generatedRoutine = await generateAIRoutine(answers);
      return generatedRoutine;
    } catch (error) {
      console.error('Error in AI routine generation:', error);
      // If API fails, fall back to the local generator as a backup
      return createEnhancedRoutine(answers);
    }
  };
